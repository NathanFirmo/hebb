...
- `retrieve`: combines structured filters, `sqlite-vec` and associative expansion.
...
- `store`: owns SQLite, `sqlite-vec`, migrations and transactions.
...
score =
  0.55 * vector_score +
  0.20 * association_score +
  0.15 * salience_strength_confidence +
  0.10 * freshness_or_temporal_fit
```
